...
namespace POEMgr.ScheduleJob
{
    public class MyJobs
    {
        private IConfiguration _configuration = CZHttpContext.ServiceProvider.GetService(typeof(IConfiguration)) as IConfiguration;
        ...
        {
            POEContext _POEContext = CZHttpContext.ServiceProvider.GetService(typeof(POEContext)) as POEContext;
            try
            {
                _POEContext.Poe_SchedulerLog.Add(new Poe_SchedulerLog
                {
                    Id = Guid.NewGuid(),
                ...
                poe_POERequests = _POEContext.Poe_POERequest.Where(x => x.CompletedDate == null && x.Status != "Submitted" && x.Status != "Approved").ToList();
                poe_MailTemplates = _POEContext.Poe_MailTemplate.Where(x => poe_POERequests.Select(y => y.IncentiveId).Contains(x.IncentiveId)).ToList();
                poe_Incentives = _POEContext.Poe_Incentive.Where(x => poe_POERequests.Select(y => y.IncentiveId).Contains(x.Id)).ToList();
                poe_Partners = _POEContext.Poe_Partner.Where(x => poe_POERequests.Select(y => y.PartnerId).Contains(x.Id)).ToList();
                poe_PartnerIncentives = _POEContext.Poe_PartnerIncentive
                    ...
                    .ToList();
                poe_MailSendRecords = _POEContext.Poe_MailSendRecord.Where(x => poe_POERequests.Select(y => y.Id).Contains(x.PoeRequestId)).ToList();
                poe_Customers = _POEContext.Poe_Customer.Where(x => poe_POERequests.Select(y => y.CustomerId).Contains(x.Id)).ToList();
                poe_SubscriptionStatuss = _POEContext.Poe_SubscriptionStatus.Where(x => poe_POERequests.Select(y => y.Id).Contains(x.PoeRequestId)).ToList();
                poe_RequestLogs = _POEContext.Poe_RequestLog.Where(x => poe_POERequests.Select(y => y.Id).Contains(x.RequestId)).ToList();
                ...
                foreach (Poe_POERequest item in poe_POERequests)
                {
                    ...
                    {
                        if (WorkDayMinus(DateTime.Now, Convert.ToDateTime(item.StartDate)) >= poe_Incentive.RemindEmailDay && item.Status == "EmailSent")
                        {
        ...
        {
            POEContext _POEContext = CZHttpContext.ServiceProvider.GetService(typeof(POEContext)) as POEContext;
            EmailServiceCore _emailService = CZHttpContext.ServiceProvider.GetService(typeof(EmailServiceCore)) as EmailServiceCore;
            try
            {
                ...
                if (poe_Partner != null && !string.IsNullOrEmpty(email))
                {
                    ...
                    if (poe_MailTemplate != null)
                    {
                        ...
                        content = content.Replace("{I}", string.IsNullOrEmpty(_configuration["LoginUrl"]) ? string.Empty : $"<a href=\"{_configuration["LoginUrl"]}\">登录</a>");
                        ...
                        //send mail
                        string sendResult = await _emailService.SendAsync(sendTo, cc, type, content);
        ...
        public async Task SendHttpRequest()
        {
            ...
            try
            {
                ...
                {
                    var responseMessage = await hc.GetAsync(_configuration["KeepRequestUrl"]);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task SendMailTest()
        {
            EmailServiceCore _emailService = CZHttpContext.ServiceProvider.GetService(typeof(EmailServiceCore)) as EmailServiceCore;
            POEContext _POEContext = CZHttpContext.ServiceProvider.GetService(typeof(POEContext)) as POEContext;
            string sendResult = await _emailService.SendAsync(_configuration["SendMailTestSendTo"], "Schedule SendMailTest", "Schedule SendMailTest");
            _POEContext.Poe_SchedulerLog.Add(new Poe_SchedulerLog
            {
                Id = Guid.NewGuid(),
